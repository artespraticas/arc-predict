    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-arc-surface border border-arc-border rounded-sm shadow-2xl">
        ...
        <div className="p-5 space-y-5">
          {/* Question */}
          <p className="text-arc-text text-sm font-body leading-relaxed">
            {market.question}
          </p>
          ...
          {step === "success" ? (
            <div className="text-center py-6 space-y-3">
              ...
              <p className="text-arc-muted text-sm">
                ...
                <span className={side === "yes" ? "text-arc-green" : "text-arc-red"}>
                  {side.toUpperCase()}
                </span>
              </p>
              {txHash && (
              ...
              <button
                onClick={onClose}
                className="w-full mt-2 py-2.5 bg-arc-accent/10 border border-arc-accent/30 text-arc-accent rounded-sm text-sm font-medium"
              >
                Close
              </button>
            </div>
            ...
            <div className="text-center py-6 space-y-3">
              ...
              <button
                onClick={() => setStep("bet")}
                className="w-full py-2.5 bg-arc-red/10 border border-arc-red/30 text-arc-red rounded-sm text-sm font-medium"
              >
                Try Again
              </button>
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => setSide("yes")}
                  className={`py-3 rounded-sm border text-sm font-medium transition-all ${
                    side === "yes"
                      ? "bg-arc-green/15 border-arc-green/60 text-arc-green glow-green"
                      : "bg-arc-bg border-arc-border text-arc-muted hover:border-arc-green/30"
                  }`}
                >
                  YES ↑
                </button>
                <button
                  ...
                  className={`py-3 rounded-sm border text-sm font-medium transition-all ${
                    side === "no"
                      ? "bg-arc-red/15 border-arc-red/60 text-arc-red glow-red"
                      : "bg-arc-bg border-arc-border text-arc-muted hover:border-arc-red/30"
                  }`}
                >
                  NO ↓
                </button>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center bg-arc-bg border border-arc-border rounded-sm overflow-hidden focus-within:border-arc-accent/50">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent py-3 px-2 text-white font-mono text-sm outline-none"
                    placeholder="10"
                  />
                ...
                <div className="flex gap-2">
                  {["5", "10", "50", "100"].map((q) => (
                    <button
                      key={q}
                      onClick={() => setAmount(q)}
                      className="flex-1 py-1 text-xs font-mono bg-arc-bg border border-arc-border hover:border-arc-accent/30 text-arc-muted hover:text-arc-accent rounded-sm transition-all"
                    >
                      ${q}
                    </button>
                  ))}
              ...
              <div className="bg-arc-bg border border-arc-border rounded-sm p-3 flex justify-between items-center">
                ...
                <span className="text-arc-gold font-mono font-medium text-sm">
                  {formatUSDC(estimatedPayout)}{" "}
                  <span className="text-arc-muted text-xs">({multiplier}×)</span>
                </span>
              </div>

              {/* Source */}
              <p className="text-xs text-arc-muted font-body">
                📋 Resolution source:{" "}
                <span className="text-arc-text">{market.resolutionSource}</span>
              </p>
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-sm text-sm font-medium flex items-center justify-center gap-2 transition-all ${
                  side === "yes"
                    ? "bg-arc-green/15 hover:bg-arc-green/25 border border-arc-green/40 text-arc-green"
                    : "bg-arc-red/15 hover:bg-arc-red/25 border border-arc-red/40 text-arc-red"
